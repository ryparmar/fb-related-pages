...
def get_related(driver, printout: bool = False) -> List[str]:
...
def save(file: str, link: str, related: List[str]):
    """Save crawled data into file.

    Arguments:
        file: output filepath
        link: source page link
        relate: list of related (recommended) pages for the source link
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--links", type=str, help="path to the file with the initial links")
    parser.add_argument(
        "--in_depth", default=False, type=str, help="if True, script will crawl also related pages of related pages"
    )
            ...
            for div in divs:
                try:
                    if div.get_attribute("aria-label") == "Liked":
                        # Set of executed actions if page is already liked
                        div.click()  # unlike
                        sleep(2)
                        div.click()  # like in order to see the offered pages
                        sleep(2)
                        related = get_related(driver)
                        save(outfile, link, related)  # save related pages of given link in the outfile

                        # append links to pages related to the initial pages
                        if args.in_depth and link in INITIAL_LINKS and c <= len(INITIAL_LINKS):
                            crawled_links += related
                            logging.info(f"{link} -- related added. Currently {len(crawled_links)} links")
                        sleep(2)
                        break  # the desired 'Like' button is listed as the first div which satisfy the if/elif condition
                    elif div.get_attribute("aria-label") == "Like":
                        # Set of executed actions if page is not yet liked
                        div.click()  # like in order to see the offered pages
                        sleep(2)
                        related = get_related(driver)
                        save(outfile, link, related)  # save related pages of given link in the outfile
